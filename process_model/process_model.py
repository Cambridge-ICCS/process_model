...
@click.option('-m', '--module-name', default='ml_module', type=click.STRING,
...
def main(model_dirs, output_file, tag_set, module_name, signature_def, indent):
    ...
    for model_dir in model_dirs:
        ...
        if signature_def not in signature_def_map:
            print(f'The SavedModel {model_dir}\ndoes not contain'
                  ' signature-def "{signature_def}".', file=sys.stderr)
            print('The model\'s MetaGraphDef contains SignatureDefs with the '
                  'following keys:', file=sys.stderr)
    ...
    for loop_index, model in enumerate(models, start=1):
        ...
        # Model definitions.
        output_str += _render_template(
                'module_definitions.F90', model_index=loop_index,
                model_dir=model.model_dir, inputs=model.inputs,
    ...
    output_str += _render_template(
        'module_body.F90', max_tags_len=Model.longest_tag_len, models=models,
        module_name=module_name,
    )
        ...
        for key, inp in sorted(inputs.items()):
            ...
            if tup:
                ...
                Model.functions_needed[
                    '{}_{}_associate_tensor'.
                    format(tup[1], len(inp['tensor_shape']))
                    ] = assoc_def_str
